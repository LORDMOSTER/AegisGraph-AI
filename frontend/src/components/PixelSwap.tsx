...
const buildGrid = ({ width, height, pixelSize, gap, pattern, randomness }: {
  width: number; height: number; pixelSize: number; gap: number; pattern: string; randomness: number;
}): Grid => {
  let size = pixelSize;
...
const buildKeyframes = ({ ease, startScale, endScale, spin, fade }: {
  ease: (p: number) => number; startScale: number; endScale: number; spin: number; fade: boolean;
}) => {
  ...
  for (let step = 0; step <= KEYFRAME_STEPS; step += 1) {
    ...
    window.push({ offset: progress, opacity: fade ? Math.min(1, eased * 1.6) : 1, transform: `rotate(${angle}deg) scale(${scale})` });
    content.push({ offset: progress, transform: `scale(${1 / scale}) rotate(${-angle}deg)` });
  }
...
function PixelSwap({
  ...
  pixelSize = 64, gap = 0, pixelRadius = 0, pixelSpin = 0, pixelScale = 0.35,
  fade = true, duration = 1400, pixelDuration = 450,
  pattern = 'random', randomness = 0,
  easing = 'cubic-bezier(0.22, 1, 0.36, 1)',
  trigger = 'hover', initialActive = false,
  active, onActiveChange, onComplete,
  aspectRatio = '16 / 10', className = '', style,
}: PixelSwapProps) {
  ...
  const grid = useMemo(() => buildGrid({ width: box.width, height: box.height, pixelSize: Math.max(8, Math.round(pixelSize)), gap: Math.max(0, Math.round(gap)), pattern, randomness }),
  ...
  useEffect(() => {
    ...
    if (!source || !frozenGrid.pixels.length || window.matchMedia('(prefers-reduced-motion: reduce)').matches) { finish(); return; }
    ...
    const keyframes = buildKeyframes({ ease: makeEasing(settings.easing), startScale: clamp(settings.pixelScale, 0.05, 1) * endScale, endScale, spin: settings.pixelSpin, fade: settings.fade });
  ...
  const interactionProps = useMemo(() => {
    ...
    if (trigger === 'click') return { onClick: () => requestActive(!desiredActive), onKeyDown: (e: React.KeyboardEvent) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); requestActive(!desiredActive); } }, role: 'button', tabIndex: 0 };
    return {};
  ...
  const renderLayer = (content: React.ReactNode, index: number) => {
    ...
    return (
      <div key={index} ref={el => { layerRefs.current[index] = el; }} className="pixel-swap__layer"
        data-visible={isShown && !(transition && index === incomingIndex)} style={{ zIndex: isShown ? 2 : 1 }} aria-hidden={!isShown}>
        {content}
      </div>
    );
  };
  ...
  return (
    <div ref={containerRef} className={`pixel-swap ${className}`.trim()} style={{ aspectRatio, ...style }} data-active={shownActive} data-transitioning={!!transition} {...interactionProps}>
      ...
      {transition && (
        <div className="pixel-swap__grid" aria-hidden="true">
          {transition.grid.pixels.map((pixel, index) => (
            <div key={pixel.id} ref={el => { pixelRefs.current[index] = el; }} className="pixel-swap__pixel"
              style={{ left: pixel.left, top: pixel.top, width: transition.grid.size, height: transition.grid.size, borderRadius: `${clamp(pixelRadius, 0, 50)}%` }} />
          ))}
        </div>
      )}
    </div>
